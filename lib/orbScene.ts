...
import type { VoiceThemeConfig } from "./voiceCommander";
...
export interface OrbSceneApi {
  ...
  zoomOut(): void;
  resetView(): void;
  /** Auto-spin the orb continuously. 0 stops auto-spin. */
  setAutoSpin(speed: number): void;
  /** Trigger an energetic visual overdrive pulse */
  triggerPulse(): void;
  /** Dynamically re-tint the 3D hologram colors and post-processing */
  setTheme(theme: VoiceThemeConfig): void;
  /** Set the active assistant emotion state (Phase 5) */
  setEmotionState(state: EmotionState): void;
  /** Pass real-time microphone or TTS audio amplitude (0..1) to dynamically pulse the core */
...
export function createOrbScene(container: HTMLElement): OrbSceneApi {
  ...
  // Emotion & Audio reactivity state
  let activeEmotion: EmotionState = "idle";
  let audioAmplitude = 0;

  // ——— SCENE ———
  const scene = new THREE.Scene();
  const camera = new THREE.PerspectiveCamera(55, Math.max(0.1, width) / Math.max(0.1, height), 0.1, 500);
  ...
  // Chromatic aberration + dynamic tone color grade shader
  const chromaticShader = {
    uniforms: {
      tDiffuse: { value: null },
      uTime: { value: 0 },
      uIntensity: { value: 0.003 },
      uTone: { value: new THREE.Vector3(1.15, 0.85, 0.55) },
    },
    vertexShader: `
      varying vec2 vUv;
      void main() {
    ...
    fragmentShader: `
      ...
      void main() {
        ...
        // Grade tone
        gl_FragColor.rgb = mix(gl_FragColor.rgb, gl_FragColor.rgb * uTone, 0.35);
      }
    `,
  };
  ...
  // ——— COLOR MANAGEMENT ———
  type ColorRole = "bright" | "mid" | "dim" | "faint" | "hot";
  const trackedMaterials: { mat: THREE.Material & { color?: THREE.Color }; role: ColorRole }[] = [];

  let C_BRIGHT = new THREE.Color(0xffaa30);
  let C_MID = new THREE.Color(0xdd7700);
  let C_DIM = new THREE.Color(0x884400);
  let C_FAINT = new THREE.Color(0x553300);
  let C_HOT = new THREE.Color(0xffcc66);

  function applyThemeColors(baseHex: number, tone: [number, number, number]) {
    const base = new THREE.Color(baseHex);
    const hsl = { h: 0, s: 0, l: 0 };
    base.getHSL(hsl);

    C_BRIGHT = new THREE.Color().setHSL(hsl.h, Math.min(1, hsl.s * 1.1), 0.58);
    C_MID = new THREE.Color().setHSL(hsl.h, hsl.s, 0.44);
    C_DIM = new THREE.Color().setHSL(hsl.h, Math.max(0.2, hsl.s * 0.9), 0.28);
    C_FAINT = new THREE.Color().setHSL(hsl.h, Math.max(0.2, hsl.s * 0.8), 0.16);
    C_HOT = new THREE.Color().setHSL(hsl.h, Math.max(0.2, hsl.s * 0.6), 0.78);

    for (const item of trackedMaterials) {
      if (item.mat.color) {
        switch (item.role) {
          case "bright":
            item.mat.color.copy(C_BRIGHT);
            break;
          case "mid":
            item.mat.color.copy(C_MID);
            break;
          case "dim":
            item.mat.color.copy(C_DIM);
            break;
          case "faint":
            item.mat.color.copy(C_FAINT);
            break;
          case "hot":
            item.mat.color.copy(C_HOT);
            break;
        }
      }
    }

    chromaticPass.uniforms.uTone.value.set(tone[0], tone[1], tone[2]);
  }
  ...
  // ——— MATERIAL HELPERS ———
  function lineMat(role: ColorRole, opacity = 1) {
    let initial = C_MID;
    if (role === "bright") initial = C_BRIGHT;
    else if (role === "dim") initial = C_DIM;
    else if (role === "faint") initial = C_FAINT;
    else if (role === "hot") initial = C_HOT;
    ...
    trackedMaterials.push({ mat, role });
    return mat;
  }
  ...
  for (let i = -15; i <= 15; i++) {
    ...
    const role: ColorRole = i % 3 === 0 ? "mid" : "faint";
  ...
  for (let i = 0; i < 4; i++) {
    ...
    for (let j = 0; j < CROSS_LINES; j++) {
      ...
      const role: ColorRole = Math.abs(t) < 0.3 ? "bright" : "mid";
      outerShell.add(
        new THREE.Line(meridian(R1, lon + offset, 200), lineMat(role, opacity)),
      );
    }
  }
  ...
  for (let j = 0; j < EQ_LINES; j++) {
    ...
    const role: ColorRole = Math.abs(t) < 0.3 ? "bright" : "mid";
    outerShell.add(
      new THREE.Line(latRing(R1, offset, 200), lineMat(role, opacity)),
    );
  }
  ...
  trackedMaterials.push({ mat: coreSphereMat, role: "hot" });
  ...
  trackedMaterials.push({ mat: glowSphereMat, role: "mid" });
  ...
  function makeTextSprite(text: string, size = 0.08) {
    ...
    const s = new THREE.Sprite(
      new THREE.SpriteMaterial({
        map: tex,
        color: C_BRIGHT.clone(),
        transparent: true,
        blending: THREE.AdditiveBlending,
        depthWrite: false,
      }),
    );
    trackedMaterials.push({ mat: s.material as any, role: "bright" });
  ...
  for (let i = 0; i < 250; i++) {
    ...
    const role: ColorRole = Math.random() > 0.7 ? "bright" : "mid";
    const mat = new THREE.MeshBasicMaterial({
      color: (role === "bright" ? C_BRIGHT : C_MID).clone(),
      transparent: true,
  ...
  trackedMaterials.push({ mat: dustMat, role: "bright" });
  ...
  function makeScanRing(radius: number, thickness = 0.015) {
    ...
    trackedMaterials.push({ mat, role: "bright" });
    const mesh = new THREE.Mesh(geo, mat);
  ...
  // Auto-spin state & pulse overdrive
  let autoSpinSpeed = 0;
  let pulseEnergy = 0;

  function setAutoSpin(speed: number) {
    autoSpinSpeed = speed;
  }
  ...
  function animate() {
    ...
    // Auto-spin logic
    if (autoSpinSpeed !== 0) {
      orbGroup.rotation.y += autoSpinSpeed;
    }
    ...
    const surge = wave3 * 1.5 + wave4 * 2.0 + pulseEnergy * 3.0;
    const coreScale = 1 + surge + Math.sin(t * 5) * 0.05;
    coreSphere.scale.setScalar(coreScale);

    const coreOpacity = Math.max(
      0,
      (0.08 + wave1 * 0.05 + surge * 0.2 + pulseEnergy * 0.4) * (1 - fadeOut * 0.95),
    );
    coreSphereMat.opacity = Math.min(0.8, coreOpacity);
    glowSphere.scale.setScalar(1 + surge * 0.8 + pulseEnergy * 1.2);
    glowSphereMat.opacity = Math.max(0, (0.03 + surge * 0.08 + pulseEnergy * 0.2) * (1 - fadeOut * 0.9));

    icoWire.scale.setScalar(1 + surge * 0.6 + pulseEnergy * 0.8);
    icoWireMat.opacity = Math.min(1, 0.5 + surge * 0.4 + pulseEnergy * 0.5);

    // Decay pulse energy smoothly
    if (pulseEnergy > 0) {
      pulseEnergy = Math.max(0, pulseEnergy - 0.02);
    }
    ...
    (scanRing1.material as THREE.MeshBasicMaterial).opacity = (0.2 + pulseEnergy * 0.3) * scanS1;
    ...
    (scanRing2.material as THREE.MeshBasicMaterial).opacity = (0.15 + pulseEnergy * 0.25) * scanS2;
    ...
    const baseScale = 1.0 + pulseEnergy * 0.15;

    // Emotion State Machine Overrides (Phase 5)
    let emotionBloomBonus = 0;
    if (activeEmotion === "speaking" || activeEmotion === "listening") {
      const audioScale = 1.0 + audioAmplitude * 0.28;
      orbGroup.scale.set(baseScale * audioScale, baseScale * audioScale, baseScale * audioScale);
      emotionBloomBonus = audioAmplitude * 1.5;
    } else if (activeEmotion === "thinking") {
      const thinkPulse = Math.sin(t * 6.0) * 0.05;
      orbGroup.scale.set(baseScale + thinkPulse, baseScale + thinkPulse, baseScale + thinkPulse);
      dustPoints.rotation.y += 0.003;
    } else if (activeEmotion === "confirm_pending") {
      // Amber heartbeat double-pulse
      const hb = Math.sin(t * 4.0) > 0.6 ? 0.08 : 0;
      orbGroup.scale.set(baseScale + hb, baseScale + hb, baseScale + hb);
      emotionBloomBonus = hb * 2.0;
    } else if (activeEmotion === "success") {
      emotionBloomBonus = 1.2;
    } else if (activeEmotion === "error") {
      // Slight glitch jitter
      orbGroup.position.x = (Math.random() - 0.5) * 0.04;
      orbGroup.position.y = (Math.random() - 0.5) * 0.04;
    } else {
      orbGroup.position.set(0, 0, 0);
    }

    // Bloom pulse + overdrive surge + emotion bonus
    bloom.strength = 1.6 + Math.sin(t * 0.8) * 0.3 + pulseEnergy * 3.0 + emotionBloomBonus;
  ...
  function setEmotionState(state: EmotionState) {
    activeEmotion = state;
  ...
  function setAudioAmplitude(level: number) {
    audioAmplitude = Math.max(0, Math.min(1, level));
  }
  ...
  return {
    ...
    setAutoSpin,
    triggerPulse,
    setTheme: (theme: VoiceThemeConfig) => applyThemeColors(theme.colorHex, theme.chromaticTone),
    setEmotionState,
    setAudioAmplitude,
    dispose,
  };
